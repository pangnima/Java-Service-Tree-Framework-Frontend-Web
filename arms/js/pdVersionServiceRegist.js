...
function setSideMenu() {
    setTimeout(function(){
        ...
        $('#sidebar_menu_product_version_regist').addClass("active");
        $('#sidebar_menu_product_version_regist').css({'color':'lightblue'});
        $('#sidebar_menu_product_version_regist').css({'font-weight':'900'});
    },1000);
}
...
function jsTreeProductBuild(){
    ...
    $("#mmenu input, #mmenu button").click(function() {
        switch (this.id) {
            case "add_default":
            case "add_folder":
                $("#productTree").jstree("create", null, "last", {
                    "attr": {
                ...
                break;
            case "search":
                $("#productTree").jstree("search", document.getElementById("text").value);
                break;
            case "text":
                break;
            default:
                $("#productTree").jstree(this.id);
                break;
        }
    });
}

// --- jstree ( version ) 테이블 설정 --- //
$(function() {
    jsTreeVersionBuild();
});

function jsTreeVersionBuild(){
    ...
    $("#versionTree").bind(
        ...
        "json_data": {
            ...
            "ajax": {
                // the URL to fetch the data
                "url": isDevelopingToRoute + "/api/arms/pdVersionService/getChildNode.do",
                // the `data` function is executed in the instance's scope
                // the parameter is the node being loaded
                // (may be -1, 0, or undefined when loading the root nodes)
                "data": function(n) {
        ...
        "search": {
            ...
            // Same as above - the `ajax` config option is actually jQuery's AJAX object
            "ajax": {
                "url": isDevelopingToRoute + "/api/arms/pdVersionService/searchNode.do",
                // You get the search string as a parameter
                "data": function(str) {
                    return {
                        "searchString": str
        ...
        $.post(isDevelopingToRoute + "/api/arms/pdVersionService/addNode.do", {
            "ref": data.rslt.parent.attr("id").replace("node_", "").replace("copy_", ""),
            "c_position": data.rslt.position,
            "c_title": data.rslt.name,
            "c_type": data.rslt.obj.attr("rel")
        ...
        data.rslt.obj.each(function() {
            $.ajax({
                async: false,
                type: 'POST',
                url: isDevelopingToRoute + "/api/arms/pdVersionService/removeNode.do",
                data: {
                    "c_id": this.id.replace("node_", "").replace("copy_", "")
                },
                success: function(r) {
        ...
        $.post(isDevelopingToRoute + "/api/arms/pdVersionService/alterNode.do", {
            "c_id": data.rslt.obj.attr("id").replace("node_", "").replace("copy_", ""),
            "c_title": data.rslt.new_name,
            "c_type": data.rslt.obj.attr("rel")
        }, function(r) {
        ...
        $.post(isDevelopingToRoute + "/api/arms/pdVersionService/alterNodeType.do", {
            "c_id": data.rslt.obj.attr("id").replace("node_", "").replace("copy_", ""),
            "c_title": data.rslt.new_name,
            "c_type": data.rslt.obj.attr("rel")
        }, function(r) {
        ...
        data.rslt.o.each(function(i) {
            $.ajax({
                async: false,
                type: 'POST',
                url: isDevelopingToRoute + "/api/arms/pdVersionService/moveNode.do",
                data: {
                    "c_id": $(this).attr("id").replace("node_", "").replace("copy_", ""),
                    "ref": data.rslt.cr === -1 ? 1 : data.rslt.np.attr("id").replace("node_", "").replace("copy_", ""),
                    "c_position": data.rslt.cp + i,
    ...
    $("#mmenu input, #mmenu button").click(function() {
        switch (this.id) {
            case "add_default":
            case "add_folder":
                $("#versionTree").jstree("create", null, "last", {
                    "attr": {
                        "rel": this.id.toString().replace("add_", "")
                    }
                });
                break;
            case "search":
                $("#versionTree").jstree("search", document.getElementById("text").value);
                break;
            case "text":
                break;
            default:
                $("#versionTree").jstree(this.id);
                break;
        }
    });
}
